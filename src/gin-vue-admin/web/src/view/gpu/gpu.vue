  ...
  <div>
    <div class="search-term">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">          
        ...
        <el-form-item>
          <el-button @click="openDialog" type="primary">新增gpu表</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <el-table-column label="库存" prop="stock" width="120"></el-table-column> 
      ...
      <el-table-column label="按钮组">
        <template slot-scope="scope">
          <el-button class="table-button" @click="updateGpu(scope.row)" size="small" type="primary" icon="el-icon-edit">变更</el-button>
...
export default {
  ...
  methods: {
      ...
      async onDelete() {
        ...
        const res = await deleteGpuByIds({ ids })
        if (res.code == 0) {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    async updateGpu(row) {
      const res = await findGpu({ ID: row.ID });
      this.type = "update";
      if (res.code == 0) {
        this.formData = res.data.regpu;
    ...
    async deleteGpu(row) {
      const res = await deleteGpu({ ID: row.ID });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功"
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case "create":
          res = await createGpu(this.formData);
          break;
        case "update":
          res = await updateGpu(this.formData);
          break;
        default:
          res = await createGpu(this.formData);
          break;
