  ...
  <div>
    <div class="search-term">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">          
        ...
        <el-form-item>
          <el-button @click="openDialog" type="primary">新增storage表</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <el-table-column label="库存" prop="stock" width="120"></el-table-column> 
      ...
      <el-table-column label="按钮组">
        <template slot-scope="scope">
          <el-button class="table-button" @click="updateStorage(scope.row)" size="small" type="primary" icon="el-icon-edit">变更</el-button>
...
export default {
  ...
  methods: {
      ...
      async onDelete() {
        ...
        const res = await deleteStorageByIds({ ids })
        if (res.code == 0) {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    async updateStorage(row) {
      const res = await findStorage({ ID: row.ID });
      this.type = "update";
      if (res.code == 0) {
    ...
    async deleteStorage(row) {
      const res = await deleteStorage({ ID: row.ID });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功"
