  ...
  <div>
    <div class="search-term">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">          
        ...
        <el-form-item>
          <el-button @click="openDialog" type="primary">新增cpu表</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <el-table-column label="类型" prop="type" width="120"></el-table-column> 
      ...
      <el-table-column label="按钮组">
        <template slot-scope="scope">
          <el-button class="table-button" @click="updateCpu(scope.row)" size="small" type="primary" icon="el-icon-edit">变更</el-button>
...
export default {
  ...
  methods: {
      ...
      async onDelete() {
        ...
        const res = await deleteCpuByIds({ ids })
        if (res.code == 0) {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    async updateCpu(row) {
      const res = await findCpu({ ID: row.ID });
      this.type = "update";
      if (res.code == 0) {
        this.formData = res.data.recpu;
    ...
    async deleteCpu(row) {
      const res = await deleteCpu({ ID: row.ID });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功"
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case "create":
          res = await createCpu(this.formData);
          break;
        case "update":
          res = await updateCpu(this.formData);
          break;
        default:
          res = await createCpu(this.formData);
          break;
