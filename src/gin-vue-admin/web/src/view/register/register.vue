<template>
  <div>
    <div class="search-term">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        ...
        <el-form-item label="权限">
          <el-input placeholder="搜索条件" v-model="searchInfo.authorityId"></el-input>
        </el-form-item>    
        <el-form-item>
          <el-button @click="onSubmit" type="primary">查询</el-button>
        </el-form-item>
        <el-form-item>
          <el-button @click="openDialog" type="primary">新增用户注册</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <el-table-column label="权限" prop="authorityId" width="120"></el-table-column> 
    
      <el-table-column label="按钮组">
        <template slot-scope="scope">
          <el-button class="table-button" @click="updateRegister(scope.row)" size="small" type="primary" icon="el-icon-edit">变更</el-button>
...
export default {
  ...
  methods: {
      ...
      async onDelete() {
        ...
        const res = await deleteRegisterByIds({ ids })
        if (res.code == 0) {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    async updateRegister(row) {
      const res = await findRegister({ ID: row.ID });
      this.type = "update";
      if (res.code == 0) {
    ...
    async deleteRegister(row) {
      const res = await deleteRegister({ ID: row.ID });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功"
